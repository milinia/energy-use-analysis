...
class FunctionCallHandler: SyntaxVisitor {
    ...
    override func visit(_ node: FunctionCallExprSyntax) -> SyntaxVisitorContinueKind {
        if isTestingFile {
            ...
            if !(node.calledExpression.description.first?.isUppercase ?? true) && !(node.parent?.description.contains("fatalError") ?? true) && !(node.parent?.description.contains("super") ?? true) && !(node.parent?.description.contains(".success") ?? true) && !(node.parent?.description.contains(".failure") ?? true) && !(node.parent?.description.contains("DispatchQueue") ?? true) && !(node.description.contains("NSLayoutConstraint.activate") ){
                ...
                var saveToFileFunctionCall = "SaveData.writeToFile(file: .functionsTimeFile, text: \"\(file?.path ?? "") \(lineStart) \(functionName) \\(endTime\(count).timeIntervalSince(startTime\(count)))\")"
                ...
                if (node.parent?.description.contains("return") ?? true) { //функция вызывается вместе с return
                    ...
                    offset += 1
                    returnCount += 1
                } else if (((node.parent?.as(LabeledExprSyntax.self)) != nil) || isLabeledExp) { //функция вызывается и передается в качестве аргумента
                    ...
                    file?.lines.insert(setValueLine, at: lineStart - 2 + offset)
                    lineStart = lineStart - 1
                    argumentCount += 1
                    offset += 1
                }
                file?.lines.insert(startTime, at: lineStart - 1 + offset)
                file?.lines.insert(endTime, at: lineEnd + 1 + offset)
                file?.lines.insert(saveToFileFunctionCall, at: lineEnd + 2 + offset)
                offset += 3
                count += 1
            }
        }
...
//            let source = node.sourceRange(converter: SourceLocationConverter(fileName: file?.path.relativePath ?? "", tree: node.root))
//            variables[source.start.line] = source.end.line
//        }
//        return .visitChildren
//    }
//    
